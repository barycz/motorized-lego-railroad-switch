...
void LwIPPacketSender::SendBeacon(udp_pcb& udp, const ip_addr_t& addr, const char* deviceName) {
...
void LwIPPacketSender::SendRequestUpdate(udp_pcb& udp, const ip_addr_t& addr) {
	static const size_t RequestUpdateSize = 32;
	uint8_t buffer[RequestUpdateSize];
	const size_t packetSize = RailwayProtocol::Packet::NewRequestUpdate(buffer, RequestUpdateSize);
	struct pbuf* p = pbuf_alloc(PBUF_TRANSPORT, packetSize, PBUF_RAM);
	memcpy(p->payload, buffer, packetSize);
	const err_t er = udp_sendto(&udp, p, &addr, RailwayProtocol::UdpPort);
	if (er != ERR_OK) {
